-- ============================================================
-- GLOBAL SAAS PLAN UPDATE FIX (v4) - ENHANCED CONTROL
-- Adds is_active, max_users, max_orders_month support
-- and introduces a secure delete function.
-- ============================================================
...
-- 2. Master RPC function with SECURITY DEFINER
...
-- 3. Delete function
CREATE OR REPLACE FUNCTION delete_saas_plan(p_id UUID) RETURNS JSON AS $$
BEGIN
  -- Security check: only SUPER_ADMIN
  IF NOT EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'SUPER_ADMIN') THEN
    RETURN json_build_object('success', false, 'error', 'Accès refusé');
